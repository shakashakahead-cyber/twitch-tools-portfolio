---
layout: ../layouts/Layout.astro
title: "サポート・お問い合わせ | Akatsukizaemon Lab"
description: "暁左衛門 Labのツールに関するサポート、不具合報告、機能要望、その他のお問い合わせ窓口です。"
---

# 📞 サポート・お問い合わせ

当サイトで公開しているツールや、記事に関するお問い合わせ窓口です。内容に合わせて以下の方法をご利用ください。

## 🚀 要望・不具合・バグ報告

開発者にとって、不具合報告や機能要望は大変ありがたいです！以下のいずれかの方法でお気軽にご連絡ください。

### 1. GitHub Issues（推奨）
開発状況が見えるため、最も確実に対応できます。GitHubアカウントをお持ちの方はこちらからお願いします。

- **[Ichi Browser Issues](https://github.com/shakashakahead-cyber/twitch-tools-portfolio/issues)**
- **[Clip Extractor Issues](https://github.com/shakashakahead-cyber/clip-extractor/issues)**
- **[Twitch Stream Helper Issues](https://github.com/shakashakahead-cyber/twitch-stream-helper/issues)**
- **[Comment Reader for Twitch Issues](https://github.com/shakashakahead-cyber/comment-Reader-for-Twitch-powered-by-Voicevox-/issues)**

Ichi Browserについては、アプリの版、Androidの版、端末名、問題を再現する手順も添えてください。パスワード、認証コード、個人情報を含む画面やURLは掲載しないでください。

### 2. X (旧Twitter)
「これってバグ？」「使い方がわからない」といった気軽な質問は、XのリプライやDMでも受け付けています。

- **[@akatsukizaemon](https://x.com/akatsukizaemon)**

---

## 📩 その他のお問い合わせ

開発依頼、コラボレーションのご相談、その他上記に当てはまらない内容は、以下のフォームまたはメールからご連絡ください。

### お問い合わせフォーム
Googleフォームより受け付けております。
[**👉 お問い合わせフォームを開く**](https://docs.google.com/forms/d/e/1FAIpQLSdoOFmj88ckLIT8YpL4QIRhARC5wcmbuqWvVbhlHfsG1juKkg/viewform?usp=dialog)

### メールでのお問い合わせ
フォームが使えない場合は、直接メールでのご連絡も可能です。
[📧 shakashakahead@gmail.com](mailto:shakashakahead@gmail.com)
