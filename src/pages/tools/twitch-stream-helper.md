...
1.  **タグプリセット保存（ロードアウト）**
    *   「FPS用」「雑談用」「レトロゲーム用」など、タグの組み合わせをカテゴリごとに保存可能。ワンクリックで呼び出せます。
2.  **スマート配信設定**
    *   配信タイトル、カテゴリ（ゲーム）、タグを一括で更新できます。ダッシュボードの重い読み込みを待つ必要はありません。
3.  **X (Twitter) 連携投稿**
    *   「今から配信開始！」の投稿をテンプレート化。配信タイトルやURLを自動で埋め込み、スムーズに告知できます。
